...
redirectPage($error);
	...
	$sql2 = "SELECT email FROM faculty WHERE email='$email'";
	$run2 = mysqli_query($con,$sql2);
	echo mysqli_num_rows($run2);
	if(mysqli_num_rows($run2)>=1){
		$error = "This user is already Added as Faculty";
       redirectPage($error);
	}else{
...
// if everything is ok, try to upload file
    ...
    if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
       ...
       $sql = "INSERT INTO `faculty` (`id`, `Name`, `email`, `password`, `mobile`, `empid`, `fname`, `address`, `img_url`, `department`) VALUES (NULL, '$name', '$email', '$password', '$mobile', '$empId', '$fname', '$address', '$img_url', '$department')";
       $run = mysqli_query($con,$sql);
       if($run){
        $_SESSION['ceoSuccess'] = "2";
        header("Location: ../home.php");
       }else{
        echo "0"; 
       }
